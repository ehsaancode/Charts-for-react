...
const QPieChart = ({
  ...
  const effectiveLegendTextColor = foreground || legendTextColor;
  const isForegroundGradient = typeof foreground === 'string' && (foreground.startsWith('linear-gradient(') || foreground.startsWith('radial-gradient('));
  const useGradientForText = useLinearGradientForeground || useRadialGradientForeground || isForegroundGradient;
  
  const getForegroundGradientCSS = () => {
    if (isForegroundGradient) {
      return foreground;
    }
    if (useRadialGradientForeground && radialGradientColorsForeground && radialGradientColorsForeground.length > 0) {
      const stops = radialGradientStopsForeground.length === radialGradientColorsForeground.length
        ? radialGradientStopsForeground
        : Array.from({ length: radialGradientColorsForeground.length }, (_, i) =>
            Math.round((i / (radialGradientColorsForeground.length - 1)) * 100)
          );
      const colorStops = radialGradientColorsForeground
        .map((color, i) => `${color} ${stops[i]}%`)
        .join(", ");
      return `radial-gradient(circle at center, ${colorStops})`;
    } else if (useLinearGradientForeground && gradientColorsForeground && gradientColorsForeground.length > 0) {
      const stops = gradientStopsForeground.length === gradientColorsForeground.length
        ? gradientStopsForeground
        : Array.from({ length: gradientColorsForeground.length }, (_, i) =>
            Math.round((i / (gradientColorsForeground.length - 1)) * 100)
          );
  ...
  const foregroundGradientCSS = useGradientForText ? getForegroundGradientCSS() : null;
  ...
  }, [data.title, baseFontSize, labelBgColor, effectiveLabelTextColor, borderRadiusX]);
  ...
  const isVerticalLayout = effectiveShowLegend && (legendPosition === "Top" || legendPosition === "Bottom");
  let chartWidth = svgWidth;
  let chartHeight = svgHeight;
  const svgPadding = 50;
  const innerWidth = Math.max(0, chartWidth - 2 * svgPadding);
        ...
        <div key={i} className="flex items-center gap-3">
          ...
          <span
            style={{
              fontSize: `${baseFontSize}px`,
              fontWeight,
              color: useGradientForText ? "transparent" : effectiveLegendTextColor,
              ...(useGradientForText && {
                backgroundImage: foregroundGradientCSS,
                WebkitBackgroundClip: "text",
                backgroundClip: "text",
  ...
  const outerJustifyClass = getJustifyClass(childAlign);
  const outerItemsClass = childAlign === "baseline" ? "items-baseline" : "items-center";
  const containerClass = `flex ${outerItemsClass} ${outerJustifyClass}`;
  ...
  const getFallbackBackgroundImage = () => {
    if (useRadialGradient && radialGradientColors && radialGradientColors.length > 0) {
      const stops = radialGradientStops.length === radialGradientColors.length
        ? radialGradientStops
        : Array.from({ length: radialGradientColors.length }, (_, i) =>
            Math.round((i / (radialGradientColors.length - 1)) * 100)
          );
      const colorStops = radialGradientColors
        ...
        .join(", ");
      return `radial-gradient(circle at center, ${colorStops})`;
    } else if (useLinearGradient && gradientColors && gradientColors.length > 0) {
      const stops = gradientStops.length === gradientColors.length
        ? gradientStops
        : Array.from({ length: gradientColors.length }, (_, i) =>
            Math.round((i / (gradientColors.length - 1)) * 100)
          );
              ...
              {useGradientForText && (
                <>
                  {isForegroundGradient && foreground.startsWith('linear-gradient') ? (
                    ...
                    <radialGradient id="fgGrad" cx="0.5" cy="0.5" r="0.5" gradientUnits="objectBoundingBox">
                      {(() => {
                        const colors = radialGradientColorsForeground;
                        const stopsArr = radialGradientStopsForeground.length === colors.length
                          ? radialGradientStopsForeground
                          : Array.from({ length: colors.length }, (_, i) =>
                              Math.round((i / (colors.length - 1)) * 100)
                            );
                        return colors.map((color, i) => (
                          <stop key={i} offset={`${stopsArr[i]}%`} stopColor={color} />
                        ));
                      })()}
                    </radialGradient>
                  ) : (
                    ...
                    >
                      {(() => {
                        const colors = gradientColorsForeground;
                        const stopsArr = gradientStopsForeground.length === colors.length
                          ? gradientStopsForeground
                          : Array.from({ length: colors.length }, (_, i) =>
                              Math.round((i / (colors.length - 1)) * 100)
                            );
                  ...
                  {originalAngle > 30 && (
                    <g>
                      ...
                      <text
                        ...
                        dominantBaseline="middle"
                        style={{
                          fill: useGradientForText ? "url(#fgGrad)" : effectiveLabelTextColor,
                          fontSize: baseFontSize,
                          fontWeight,
                        }}
                      >
          ...
          {effectiveShowTooltip && hoveredPoint && (
            <div
              ...
              style={{
                ...
                borderRadius: `${pieTooltipborderRadiusValueTopLeft || pieTooltipborderRadiusType || 0}px ${pieTooltipborderRadiusValueTopRight || pieTooltipborderRadiusType || 0}px ${pieTooltipborderRadiusValueBottomRight || pieTooltipborderRadiusType || 0}px ${pieTooltipborderRadiusValueBottomLeft || pieTooltipborderRadiusType || 0}px`,
                padding: '12px',
                boxShadow: effectiveShowTooltipShadow ? '0 4px 14px rgba(0, 0, 0, 0.3)' : 'none',
